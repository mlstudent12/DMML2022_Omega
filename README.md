...
## Data
This project involves two datasets: `training_data` and `unlabelled_test_data`. The `training_data` has three features: id, sentence, and difficulty. The `unlabelled_test_data` has only two features: id and sentence. Our goal is to use several models to predict the difficulty level for the `unlabelled_test_data`, aiming for the highest accuracy score possible.
